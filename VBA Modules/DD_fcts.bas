...
GetColumnName = Replace(Replace(Cells(1, Found.Column + position).Address, "1", ""), "$", "") ' position 0 if you need the actuall column, 1 if you need the second etc
Else
GetColumnName = ""
End If
End Function

Function FindAstring(ByVal activerange As Range, lookfor As String, off As Integer)


Set Found = activerange.Find(what:="*" & lookfor & "*", LookIn:=xlValues)
If Not Found Is Nothing Then
Set FoundString = Found.Offset(columnoffset:=off)
FindAstring = FoundString
Else
FindAstring = "No " & lookfor
End If
End Function
Function GetColumnNamebyrange(ByVal fixrange As Range, position As Integer, lookfor As String) ' rownb is for the row number where you're looking for the match

Set Found = fixrange.Find(what:="*" & lookfor & "*", LookIn:=xlValues, lookat:=xlWhole)

If Not Found Is Nothing Then
GetColumnNamebyrange = Replace(Replace(Cells(1, Found.Column + position).Address, "1", ""), "$", "") ' position 0 if you need the actuall column, 1 if you need the second etc
Else
